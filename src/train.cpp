...
void printData(int epoch, int epochs, double running_loss, size_t batch_index,
...
bool hasModelFiles(const std::string &folderPath) {
    std::filesystem::path path(folderPath);
    return std::filesystem::exists(path) &&
           std::filesystem::exists(path / "model.pt");
}
...
MyDataset::MyDataset(torch::Tensor images, torch::Tensor labels)
...
Predictor::Predictor(std::string model_path)
